...
export default function GalleryGrid({
    ...
    if (posts === undefined) {
        return (
            <div className="flex flex-col gap-8 px-4 pb-20 max-w-2xl mx-auto">
                {[1, 2, 3].map((i) => (
                    <div
                        key={i}
                        className="w-full h-[50vh] bg-primary/10 rounded-3xl animate-pulse"
                    />
                ))}
            </div>
        );
        ...
        >
            {posts.map((post, index) => (
                <motion.div
                    ...
                    className="relative rounded-3xl overflow-hidden flex flex-col group cursor-zoom-in shadow-xl bg-neutral-100 dark:bg-neutral-800"
                    onClick={() => onPostClick?.(post)}
                >
                    <div className="relative overflow-hidden">
                        {post.mediaType === 'video' ? (
                            <div
                                className="relative w-full flex items-center justify-center overflow-hidden bg-black/5"
                                style={{
